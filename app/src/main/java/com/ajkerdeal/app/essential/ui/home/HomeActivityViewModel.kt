...
class HomeActivityViewModel(private val repository: AppRepository): ViewModel() {
    ...
    fun updateMerchantLocation(requestBody: MerchantLocationRequest): LiveData<Boolean> {

        val responseData: MutableLiveData<Boolean> = MutableLiveData()
        viewModelScope.launch(Dispatchers.IO){
            val response = repository.updateMerchantLocation(requestBody)
            withContext(Dispatchers.Main) {
                //viewState.value = ViewState.ProgressState(false)
                when (response) {
                    is NetworkResponse.Success -> {
    ...
    fun updateLocationAD(requestBody: LocationUpdateRequestAD): LiveData<Boolean> {

        val responseData: MutableLiveData<Boolean> = MutableLiveData()
        viewModelScope.launch(Dispatchers.IO){
            val response = repository.updateLocationUpdateRequestAD(requestBody)
            withContext(Dispatchers.Main) {
                //viewState.value = ViewState.ProgressState(false)
                when (response) {
                    is NetworkResponse.Success -> {
    ...
    fun updateLocationDT(requestBody: LocationUpdateRequestDT): LiveData<Boolean> {

        val responseData: MutableLiveData<Boolean> = MutableLiveData()
        viewModelScope.launch(Dispatchers.IO){
            val response = repository.updateLocationUpdateRequestDT(requestBody)
            withContext(Dispatchers.Main) {
                //viewState.value = ViewState.ProgressState(false)
                when (response) {
                    is NetworkResponse.Success -> {
                        responseData.value = response.body.model.id != 0
                    }
                    is NetworkResponse.ServerError -> {
                        val message = "দুঃখিত, এই মুহূর্তে আমাদের সার্ভার কানেকশনে সমস্যা হচ্ছে, কিছুক্ষণ পর আবার চেষ্টা করুন"
                        //viewState.value = ViewState.ShowMessage(message)
    ...
    fun updateStatusLocation(requestBody: StatusLocationRequest): LiveData<Boolean> {

        val responseData: MutableLiveData<Boolean> = MutableLiveData()
        viewModelScope.launch(Dispatchers.IO){
            val response = repository.updateStatusChangeLocation(requestBody)
            withContext(Dispatchers.Main) {
                //viewState.value = ViewState.ProgressState(false)
                when (response) {
                    is NetworkResponse.Success -> {
